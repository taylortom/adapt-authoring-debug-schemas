...
class DebugSchemasModule extends AbstractModule {
  ...
  async init() {
    ...
    const router = server.api.createChildRouter('schema', [{
      route: '/list',
      handlers: { post: this.handleSchemaList.bind(this) }
    }, {
      route: '/update',
      handlers: { get: this.handleSchemaUpdate.bind(this) }
    }, {
      route: '/:schemaName',
    ...
    auth.secureRoute(`${router.path}/update`, 'POST', ['debug'])
  }
  ...
  async handleSchemaUpdate(req, res, next) {
    const schema = await this.app.waitForModule('jsonschema')
