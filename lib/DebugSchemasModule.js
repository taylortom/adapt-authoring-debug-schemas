...
class DebugSchemasModule extends AbstractModule {
  ...
  async handleSchema(req, res, next) {
    ...
    s.extensions = schema.schemaExtensions[schemaName]
    s.ancestors = []
    let parent = s
    while(parent?.$merge) {
      parent = schema.schemas[parent.$merge.source?.$ref]?.raw
      if(parent) s.ancestors = [parent.$anchor, ...s.ancestors]
    }
    if(schemaName !== 'base') s.ancestors = ['base', ...s.ancestors]
